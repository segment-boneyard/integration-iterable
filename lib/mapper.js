...
exports.identify = function(identify) {
  ...
  var iterableIntegrationFields = pick(['mergeNestedObjects'], identify.options('Iterable'));
  return extend(payload, iterableIntegrationFields);
};
...
exports.track =
// TODO - how come we need to do this for screen, but not for page?
exports.screen = function(track){
...
function dateToIterableDateStringFormat(date) {
  if (typeof date.getDate === 'function' && !Number.isNaN(date.getDate())) return date.toISOString().replace('T', ' ').split('.')[0] + " +00:00";
}
