...
/**
 * We're interested in everything, except identify calls without emails.
 */

Iterable.ensure(function(msg){
  var isIdentify = msg.type() === 'identify';
  var email = msg.proxy('traits.email');
  if (isIdentify && !email) return this.reject('expect identify calls to contain an email address');
  return;
});
